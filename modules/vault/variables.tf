...
variable "vault_telemetry_config" {
  ...
  validation {
    ...
    error_message = "Telemetry config must be provided as a map of key-value pairs (strings, booleans, or numbers)."
  }
}
...
variable "telegraf_version" {
  type        = string
  description = "Version of Telegraf to install for Vault monitoring"
  default     = "1.38.0"
}

variable "enable_telegraf_monitoring" {
  type        = bool
  description = "Boolean to enable the Telegraf + Azure Monitor monitoring addon."
  default     = false
}
...
variable "telegraf_config_template" {
  ...
  validation {
    condition     = var.telegraf_config_template != null ? fileexists("${path.cwd}/templates/${var.telegraf_config_template}") : true
...
variable "telegraf_install_template" {
  type        = string
  description = "Name of Telegraf install template file. File must exist within a directory named `./templates` within your current working directory."
  default     = null

  validation {
    condition     = var.telegraf_install_template != null ? fileexists("${path.cwd}/templates/${var.telegraf_install_template}") : true
    error_message = "File not found. Ensure the file exists within a directory named `./templates` within your current working directory."
  }
}
#------------------------------------------------------------------------------
