...
listener "tcp" {
  address       = "$HOSTNAME:${vault_port_api}"
...
%{ if lower(tostring(value)) == "true" ~}
  ${key} = true
%{ else ~}
%{ if lower(tostring(value)) == "false" ~}
  ${key} = false
%{ else ~}
...
%{ if enable_telegraf_monitoring ~}
  log "INFO" "Installing Telegraf for Vault monitoring"
  install_telegraf
%{ endif ~}
